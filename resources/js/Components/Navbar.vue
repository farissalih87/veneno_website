    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex items-center justify-between h-20">
        ...
        <nav class="hidden lg:flex items-center gap-7">
          <Link :href="`/${currentLocale}`" class="text-sm font-semibold text-zinc-300 hover:text-white transition-colors">
            {{ t('nav.home') }}
          </Link>

          <a :href="`/${currentLocale}#about`" class="text-sm font-semibold text-zinc-300 hover:text-white transition-colors">
            {{ t('nav.about') }}
          </a>

          <!-- Services Dropdown -->
          <div class="relative" @mouseenter="isServicesDropdownOpen = true" @mouseleave="isServicesDropdownOpen = false">
            <a :href="`/${currentLocale}#services`" class="flex items-center gap-1.5 text-sm font-semibold text-zinc-300 hover:text-white transition-colors py-2">
            ...
            <div v-if="isServicesDropdownOpen" class="absolute top-full -left-10 w-96 py-2 glass-panel rounded-2xl shadow-2xl shadow-black/90 border border-zinc-800 animate-in fade-in slide-in-from-top-2 duration-150 grid grid-cols-1 divide-y divide-zinc-800/60 max-h-[460px] overflow-y-auto">
              ...
              >
                <div>
                  ...
                  <div class="text-[10px] text-zinc-500 font-mono">{{ t('about.tag') }}</div>
                </div>
          ...
          <a :href="`/${currentLocale}#gallery`" class="text-sm font-semibold text-zinc-300 hover:text-white transition-colors">
            {{ t('nav.works') }}
          </a>

          <a :href="`/${currentLocale}#contact`" class="text-sm font-semibold text-zinc-300 hover:text-white transition-colors">
            {{ t('nav.contact') }}
          </a>
        </nav>

        <!-- Right Side: Language Switcher & 3 Header CTAs -->
        <div class="hidden sm:flex items-center gap-3">
          
          <!-- Compact Language Switcher Pill -->
          <div class="flex items-center p-1 rounded-xl bg-zinc-900/90 border border-zinc-800 text-xs font-mono">
            <button
              @click="switchLanguage('en')"
              class="px-2.5 py-1 rounded-lg transition-all text-xs"
              :class="currentLocale === 'en' ? 'bg-red-600 text-white font-bold shadow-md shadow-red-600/30' : 'text-zinc-400 hover:text-white'"
            >
              EN
            </button>
            <button
              ...
              class="px-2.5 py-1 rounded-lg transition-all text-xs"
              :class="currentLocale === 'ar' ? 'bg-red-600 text-white font-bold shadow-md shadow-red-600/30' : 'text-zinc-400 hover:text-white'"
            >
              العربية
            </button>
          </div>
