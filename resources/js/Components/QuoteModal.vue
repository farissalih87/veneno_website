...
import { X, Send, CheckCircle2, Phone, Mail, User, ShieldCheck, Sparkles, MapPin } from 'lucide-vue-next';
...
const props = defineProps({
  ...
  preselectedService: {
    type: String,
    default: 'Paint Protection Film (PPF)',
  },
});
...
const handleSubmit = async () => {
  ...
  try {
    ...
    errorMessage.value = err.response?.data?.message || 'Unable to submit quotation request. Please reach us directly on WhatsApp or +971 2 634 4403.';
  } finally {
...
<template>
  ...
  >
    ...
    <!-- Modal Card -->
    <div
      class="relative w-full max-w-xl glass-panel rounded-3xl border border-zinc-700/80 shadow-2xl p-6 sm:p-8 z-10 my-8 animate-in fade-in zoom-in-95 duration-200"
    >
      <!-- Close Button -->
      <button
        @click="handleClose"
